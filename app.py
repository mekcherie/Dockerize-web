...
# app.config["MONGO_URI"] = "mongodb+srv://meklitcherie:YASIkiya.7@cluster123.a4so6.mongodb.net/dbname?retryWrites=true&w=majority"
# mongo = PyMongo(app)
load_dotenv()
...
MONGODB_DBNAME = 'mydb'
client = MongoClient(f"mongodb+srv://{MONGODB_USERNAME}:{MONGODB_PASSWORD}@cluster123.a4so6.mongodb.net/{MONGODB_DBNAME}?retryWrites=true&w=majority", ssl_cert_reqs=ssl.CERT_NONE)
...
def create():
    ...
    if request.method == 'POST':
        ...
        insert_new_plant = db.plants.insert_one(new_plant)
        return redirect(url_for('detail', plant_id=insert_new_plant.inserted_id))
...
def harvest(plant_id):
    ...
    insert_new_harvest = db.harvests.insert_one(new_harvest)
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        ...
        plant_update = db.plants.update_one({'_id': ObjectId(plant_id)},
        {
            '$set': {
                'name': request.form.get('plant_name'),
                'variety': request.form.get('variety'),
...
def delete(plant_id):
    ...
    # id.delete_plant = db.plants.delete_one({'_id': ObjectId(plant_id)})
    delete_plant = db.plants.delete_one({'_id': ObjectId(plant_id)})

    # TODO: Also, make a `delete_many` database call to delete all harvests with
    # the given plant id.
    delete_harvest = db.harvests.delete_many({'plant_id': plant_id})
